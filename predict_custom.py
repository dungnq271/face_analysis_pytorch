...
class Predictor:
    ...
    def predict(self, inputs, logits=True):
        race_outputs = None
        gender_outputs = None
        age_outputs = None
        ...
        with torch.no_grad():
            ...
            if self.race_model:
                race_outputs = self.get_predictions(
                    inputs, self.race_model, self.race_head, race_outputs
                )

            if self.gender_model:
                gender_outputs = self.get_predictions(
                    inputs, self.gender_model, self.gender_head, gender_outputs
                )

            if self.age_model:
                age_outputs = self.get_predictions(
                    inputs, self.age_model, self.age_head, age_outputs
                )
